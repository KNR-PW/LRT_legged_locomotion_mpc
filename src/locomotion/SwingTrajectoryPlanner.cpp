...
namespace legged_locomotion_mpc
{
  namespace locomotion
  {
      ...
      const ModeSchedule& modeSchedule) 
    {
      ...
      const auto feetPositions = kinematicsModel_->feetPositionsInOriginFrame(
        ...
        for (int leg = 0; leg < NUM_CONTACT_POINTS; leg++) {
            ...
            heuristicFootholdsPerLeg_[leg] = selectHeuristicFootholds(leg, contactTimings, targetTrajectories, initTime,
                                                                      currentState, finalTime);

            // Select terrain constraints based on the heuristic footholds.
            nominalFootholdsPerLeg_[leg] = selectNominalFootholdTerrain(
                leg, contactTimings, heuristicFootholdsPerLeg_[leg], targetTrajectories,
    ...
    auto SwingTrajectoryPlanner::generateSwingTrajectories(int leg, const std::vector<ContactTiming> &contactTimings,
        ...
        if (startsWithSwingPhase(contactTimings)) {
            SwingPhase::SwingEvent liftOff{
                lastContacts_[leg].first, settings_.liftOffVelocity, &lastContacts_[leg].second
            };
            SwingPhase::SwingEvent touchDown = [&] {
                if (touchesDownAtLeastOnce(contactTimings)) {
                    return SwingPhase::SwingEvent{
                        contactTimings.front().start, settings_.touchDownVelocity,
        ...
        for (int i = 0; i < contactTimings.size(); ++i) {
            ...
            SwingPhase::SwingEvent touchDown = [&] {
                ...
                if (nextContactExists) {
                    return SwingPhase::SwingEvent{
                        contactTimings[i + 1].start, settings_.touchDownVelocity,
    ...
    std::pair<std::vector<scalar_t>, std::vector<std::unique_ptr<FootPhase> > >
    SwingTrajectoryPlanner::extractSwingTrajectoriesFromReference(
    ...
    std::unique_ptr<ExternalSwingPhase> SwingTrajectoryPlanner::extractExternalSwingPhase(int leg, scalar_t liftOffTime,
        ...
        return std::unique_ptr<ExternalSwingPhase>(
            new ExternalSwingPhase(move(time), move(positions), move(velocities)));
    }
    ...
    std::vector<vector3_t> SwingTrajectoryPlanner::selectHeuristicFootholds(
        ...
        const auto initBaseLinearVelocityInWorld = rotateVectorBaseToOrigin(
            getLinearVelocity(initBaseTwistInBase), initBaseOrientation);
        ...
        const base_coordinate_t initDesiredBaseTwistInBase = initDesiredState.segment<BASE_COORDINATE_SIZE>(
            BASE_COORDINATE_SIZE);
        ...
        scalar_t zmpX = pendulumFrequency * (initBaseLinearVelocityInWorld.x() - initDesiredBaseLinearVelocityInWorld.
                                             x());
        scalar_t zmpY = pendulumFrequency * (initBaseLinearVelocityInWorld.y() - initDesiredBaseLinearVelocityInWorld.
                                             y());
        ...
        for (const auto &contactPhase: contactTimings) {
            if (hasStartTime(contactPhase)) {
                ...
                vector3_t referenceFootholdPositionInWorld = kinematicsModel_->footPositionInOriginFrame(
                    leg, desiredBasePose, desiredJointPositions);
                ...
                if (timeTillContact < settings_.previousFootholdTimeDeadzone && previousIterationContact != nullptr) {
                    ...
                    if (previousIterationContact != nullptr) {
                        referenceFootholdPositionInWorld =
                                filterFoothold(referenceFootholdPositionInWorld,
    ...
    void SwingTrajectoryPlanner::updateLastContact(int leg, scalar_t expectedLiftOff,
    ...
    scalar_t SwingTrajectoryPlanner::getContactEndTime(const ContactTiming &contactPhase, scalar_t finalTime) const {
        return hasEndTime(contactPhase)
                   ? contactPhase.end
