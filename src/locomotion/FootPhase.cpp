...
namespace legged_locomotion_mpc
{
  namespace locomotion
  {
      ...
      size_t numberOfConstraints = stanceTerrain.getBoundryPoints().size();
      ...
      if (numberOfConstraints > 0) 
        ...
        for (size_t i = 0; i < numberOfConstraints; ++i) 
          ...
          const auto boundryPoint = stanceTerrain.getBoundryPoint(i);
          const auto nextBoundryPoint = stanceTerrain.getNextBoundryPoint(i);
          vector2_t start2end = nextBoundryPoint - boundryPoint;

          // Get 2D constraint A_terrain * p_terrain + b - margin >= 0
          vector2_t A_terrain{-start2end.y(), start2end.x()};
          // rotate 90deg around z to get a vector pointing to interior of the polygon
          A_terrain.normalize(); // make it a unit vector.
          scalar_t b_terrain = -A_terrain.dot(boundryPoint) - margin;
          // Take offset of one of the points
    ...
    SwingPhase::SwingPhase(SwingEvent liftOff, SwingEvent touchDown,
    ...
    void SwingPhase::setFullSwing(const SwingProfile &swingProfile,
      ...
      if (swingVector.head<2>().norm() > 0.01) 
      {
        ...
        if (terrainModel != nullptr) 
        {
          ...
          for (const auto &node: swingProfile.nodes) 
          {
            ...
            midPoint.position = liftOffPositionInWorld + node.tangentialProgress * swingVector 
              + (maxObstacleHeight + node.swingHeight) * swingTrajectoryNormal;
        ...
        {
          // (cases where target is above or intersecting with current position)
    ...
    void SwingPhase::setHalveSwing(const SwingProfile &swingProfile, const TerrainModel *terrainModel) {
      ...
      const vector3_t touchDownPositionInWorld = liftOffPositionInWorld + vector3_t(
        0.0, 0.0, swingProfile.nodes.back().swingHeight);
