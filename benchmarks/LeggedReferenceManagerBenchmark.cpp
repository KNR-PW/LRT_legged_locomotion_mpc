...
static void LeggedReferenceManager_PRESOLVE(benchmark::State & state)
{
  ...
  std::shared_ptr<SwingTrajectoryPlanner> swingPlanner = 
    std::make_shared<SwingTrajectoryPlanner>(modelInfo, swingStaticSettings, 
      swingDynamicSettings, forwardKinematics);

  std::shared_ptr<JointTrajectoryPlanner> jointPlanner = 
    std::make_shared<JointTrajectoryPlanner>(modelInfo, std::move(kinematicsSolver));

  std::shared_ptr<planners::ContactForceWrenchTrajectoryPlanner> forcePlanner = 
    std::make_shared<planners::ContactForceWrenchTrajectoryPlanner>(modelInfo);
