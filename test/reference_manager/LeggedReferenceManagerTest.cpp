...
TEST(LeggedReferenceManagerTest, getEndEffectorTrajectoryPoint) 
{
  ...
  std::vector<scalar_t> alphaValues{0.0, 0.1, 0.2, 0.5, 0.75, 0.9, 1.0};
  for(const auto alpha: alphaValues)
  {
    scalar_t testTime = 0.0;
    scalar_t testDeltaTime = managerSettings.maximumReferenceSampleInterval * alpha;
    
    while((testTime + testDeltaTime) < finalTime)
    {
      const auto point1 = manager.getEndEffectorTrajectoryPoint(testTime);
      const auto point2 = manager.getEndEffectorTrajectoryPoint(testTime + managerSettings.maximumReferenceSampleInterval);
