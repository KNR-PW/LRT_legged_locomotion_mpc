...
#include <legged_locomotion_mpc_ros2/demos/meldog_demo/meldog_definitions.hpp>
...
int main(int argc, char* argv[]) 
{
  ...
  const scalar_t heightScale = 0.10;

  auto gridMap = convex_plane_decomposition::loadGridmapFromImage(
      terrainFilePath, terrain_model::elevationLayerName, modelSettings.worldLinkName,
      perceptionConfig.preprocessingParameters.resolution, heightScale);
  gridMap.get(terrain_model::elevationLayerName).array() -=
      gridMap.atPosition(terrain_model::elevationLayerName, {0., 0.});

  auto planarTerrain = computePlanrTerrain(std::move(gridMap), perceptionConfig);

  std::unique_ptr<TerrainModel> terrainModel =
    std::make_unique<SegmentedPlanesTerrainModel>(std::move(planarTerrain), 
      modelSettings.worldLinkName);
  ...
  initialBasePosition.z() += baseSettings.initialBaseHeight;
